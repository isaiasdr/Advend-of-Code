...
for (const passports of textArray) {
    let passport = passports.split(' ');
    requiredData = {
        byr: false,
        iyr: false,
        eyr: false,
    ...
    for (const data of passportData) {
        keyValue = data.split(':');
        if (keyValue[0] === "byr") {
            if (keyValue[1].length === 4 && (keyValue[1] >= 1920 && keyValue[1] <= 2002)) {
                requiredData.byr = true;
            }
        } else if (keyValue[0] === "iyr") {
            if (keyValue[1].length === 4 && (keyValue[1] >= 2010 && keyValue[1] <= 2020)) {
                requiredData.iyr = true;
            }
        } else if (keyValue[0] === "eyr") {
            if (keyValue[1].length === 4 && (keyValue[1] >= 2020 && keyValue[1] <= 2030)) {
                requiredData.eyr = true;
            }
        } else if (keyValue[0] === "hgt") {
            if (keyValue[1].length === 5) {
                let metricUnit = keyValue[1][3]+keyValue[1][4];
                if (metricUnit === "cm") {
                    if (keyValue[1].split('cm')[0] >= 150 && keyValue[1].split('cm')[0] <= 193) {
                        requiredData.hgt = true;
                    }
                }
            } else if (keyValue[1].length === 4) {
                let metricUnit = keyValue[1][2]+keyValue[1][3];
                if (metricUnit === "in") {
                    if (keyValue[1].split('in')[0] >= 59 && keyValue[1].split('in')[0] <= 76) {
                        requiredData.hgt = true;
                    }
                }
            }
            ...
            if (keyValue[1][0] === "#") {
                if (keyValue[1].split('#')[1].length === 6) {
                    for (let i = 1; i < keyValue[1].length; i++) {
                        if (keyValue[1][i] >= 0 && keyValue[1][i] <= 9 || keyValue[1][i] >= 'a' && keyValue[1][i] <= 'f') {
                            requiredData.hcl = true;
                        }
                    }
                }
            }
        } else if (keyValue[0] === "ecl") {
            if (keyValue[1] === "amb" || keyValue[1] === "blu" || keyValue[1] === "brn" 
             || keyValue[1] === "gry" || keyValue[1] === "grn" || keyValue[1] === "hzl" 
             || keyValue[1] === "oth") {
                requiredData.ecl = true;
            }
        } else if (keyValue[0] === "pid") {
            if (keyValue[1].length === 9) {
                requiredData.pid = true;
            }
        } else if (keyValue[0] === "cid") {
            requiredData.cid = true;
        }
    }

    if (requiredData.byr === true && requiredData.iyr === true && requiredData.eyr === true
         && requiredData.hgt === true && requiredData.hcl === true && requiredData.ecl === true
         && requiredData.pid === true) {
        count++;
    }
}
