...
for (const rule of processedInput) {
    let bags = [];
    let processedRule = rule.split('contain');
    let outerBag = processedRule[0];
    outerBag = outerBag.substring(0, outerBag.indexOf(" bag"));

    let innersBags = processedRule[1];

    if (innersBags.substring(1, innersBags.length) === 'no other bags.') {
        bags.push({quantity: 0, bag: 'none'});
        rules.push({outer: outerBag, inners: bags});
        continue;
    }
    
    innersBags = innersBags.split(',');
    
    for (let innerBag of innersBags) {
        innerBag = innerBag.substring(1, innerBag.length);
        
        let quantity = Number(innerBag[0]);
        let bag;
        if (innerBag.indexOf(" bag")) {
            bag = innerBag.substring(2, innerBag.indexOf(" bag"));
        }
        bags.push({quantity: quantity, bag: bag});
    }
    rules.push({outer: outerBag, inners: bags});
}

let myBag = [{quantity: 1, bag: "shiny gold"}];

function countBags(rules, toSearch) {
    let counter = 0;

    //recorro las reglas
    for (const rule of rules) {
        //recorro el vector de bolsos a buscar
        for (const bag of toSearch) {
            //si el bolso externo es igual al que busco, consultare los internos
            if (rule.outer === bag.bag && bag.quantity) {
                let arraySearch = [];
                let temp = true;
                //recorro los bolsos internos
                for (const innerBag of rule.inners) {
                    //si hay bolsos internos debo buscarlos, se suma la cantidad multiplicada anterior
                    //y se calcula la cantidad de bolsos internos como los que hay por la cantidad de bolsos externos
                    //, sino al cantidad es el bolso exterior
                    if (innerBag.quantity) {
                        if (temp) {
                            counter+=bag.quantity;
                            temp = false;
                        }
                        arraySearch.push({quantity: bag.quantity*innerBag.quantity, bag:innerBag.bag});
                    } else {
                        counter += bag.quantity;   
                    }
                }
                if (arraySearch.length) {
                    counter += countBags(rules, arraySearch);
                }
            }
        }
    }
...
let result = countBags(rules, myBag);
result = result - myBag[0].quantity;
console.log(result);