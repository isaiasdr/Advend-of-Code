...
function addBags(rules, toSearch, listResult) {

    let arraySearch = [];

    //recorro todas las reglas
    for (const rule of rules) {
        //recorro los bolsos que puede contener
        for (const inner of rule.inners) {
            //recorro los bolsos a buscar
            for (const bagSearch of toSearch) {
                //si el bolso que busco es un bolso interno de otro
                if (bagSearch === inner.bag) {
                    let temp = false;
                    for (const outer of listResult) {
                        if (outer === rule.outer) {
                            temp = true;
                            break;
                        }
                    ...
                    if (!temp) {
                        listResult.push(rule.outer);
                        arraySearch.push(rule.outer);
                    }
                }
            }
        }
    ...
    if (arraySearch.length) {
        result = addBags (rules, arraySearch, listResult);
    }

    return listResult;
}

arrayResult = addBags(rules, toSearch, arrayResult);
