...
for (const text of input) {
    ...
    let police = separacion[0].split("-");
    let letter = separacion[1][0];
    let password= separacion[2];
    
    let countLetters= 0;
    for (const compare of password) {
        if (compare === letter) {
            countLetters++;
        }
    }

    if (countLetters >= police[0] && countLetters <= police[1]) {
        count++;
    }
}
